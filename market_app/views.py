...
def comment(request,pk):
    ...
    if request.method == 'POST':
        ...
        if form_rate.is_valid():
            ...
            freshness = form_rate.cleaned_data['freshness']
            deliveries = form_rate.cleaned_data['deliveries']
            rate = Rate()
            rate.grocery = image
            rate.user = current_user
            rate.prices = prices
            rate.freshness = freshness
            rate.deliveries = deliveries
            rate.average = (rate.prices + rate.freshness + rate.deliveries)/3
            rate.save()
            return HttpResponseRedirect(request.path_info)
    else:
...
def add_to_cart(request, **kwargs):
    ...
    # create order item of the selected grocery
